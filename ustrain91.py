...
def compile_input(n_weeks_predict, combine=False, i_sss=False, i_precip=False, i_time=False):
    ...
    location_precip_file = 'train-data/new-usprecip.pickle'  # 514 x 1299
...
def main():
    ...
    for i in range(n_random_tries):
        ...
        model, history, val_metrics = train(*reshape_data(input_data, output_data),
        ...
        model_name = 'train-results/new91_tmp_k_model_' + str(i) + '_time=' + str(input_set) + '.h5'
    ...
    for i in range(len(best_inds)):
        ...
        model_name = 'train-results/new91_k_model_' + str(i) + '_time=' + str(input_set) + '.h5'
        copyfile(models[best_inds[i]], model_name)

    # Clean out temp files
    tmp_models = glob.glob('train-results/new91_tmp_k_model_*.h5')
