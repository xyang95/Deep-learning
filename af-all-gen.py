...
for i in range(points.shape[0]):
    ...
    while date < datetime(2011, 1, 1):
        if date.year != year:
            year = date.year
            precip_ds = Dataset('raw-data/euprecip/precip.' + str(year) + '.nc')
...
with open('train-data/afapcp-daily-mean.pickle', 'wb') as fd:
    pickle.dump(P_mean, fd)

with open('train-data/afprecip-mean.pickle', 'wb') as fd:
...
with open('train-data/afprecip.pickle', 'wb') as fd:
