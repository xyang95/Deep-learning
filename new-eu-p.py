...
def compile_input(datestr, combine=False, i_sss=False, i_precip=False, i_time=False):
    ...
    training_location_precip_file = 'train-data/euprecip.pickle' # 514 x 1299
...
def main():
    ...
    model_names = glob.glob('train-results/neweu_k_model_*.h5')

    N_models = len(model_names)

    precip_wk34_predictions = np.zeros((N_models,1598))
    precip_wk56_predictions = np.zeros((N_models,1598))
    for i in range(N_models):
        ...
        prediction_i = np.reshape(prediction_i,(4,1598))
